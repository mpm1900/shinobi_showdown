...
const EFFECTIVENESS_DATA: Record<number, { color: string; size: string; label: string }> = {
...
const NatureEffectivenessTable = memo(function NatureEffectivenessTable() {
  return (
    <div className="flex flex-col gap-2 pt-4">
        ...
        {NATURES.map((a) => (
          <div key={a} className="grid grid-cols-10 col-span-10 gap-1">
            ...
            {NATURES.map((b) => {
              ...
              return (
                <div key={b} className="grid place-items-center h-8">
                  {config && (
                    <div
      ...
      <div className="flex flex-wrap justify-center gap-4 text-[10px] text-muted-foreground border-t border-white/5 pt-2">
        {Object.entries(EFFECTIVENESS_DATA).sort((a, b) => Number(b[0]) - Number(a[0])).map(([val, config]) => (
